...
@test "tlsInit/Job: can overwrite CA secret with the provided one" {
  ...
  # uses the provided ca cert to generate server cert
  actual=$(echo $spec | jq -r '.containers[0].command | join(" ") | contains("-ca=/consul/tls/ca/cert/key")' | tee /dev/stderr)
  [ "${actual}" = "true" ]

  # uses the provided ca key to generate server cert
  actual=$(echo $spec | jq -r '.containers[0].command | join(" ") | contains("-key=/consul/tls/ca/key/key")' | tee /dev/stderr)
  [ "${actual}" = "true" ]
}
